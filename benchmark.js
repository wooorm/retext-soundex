...
retext = new Retext();
retextWithSoundex = new Retext().use(soundex);
...
suite('retext w/o retext-soundex', function () {
...
suite('retext w/ retext-soundex', function () {
    bench('A paragraph (5 sentences, 100 words)', function (done) {
        retextWithSoundex.parse(paragraph, done);
    });

    bench('A section (10 paragraphs, 50 sentences, 1,000 words)',
        function (done) {
            retextWithSoundex.parse(section, done);
        }
    );
});
